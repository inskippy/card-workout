...
export default class Workout extends React.Component {
    ...
    render() {
        const history = this.props.appstate.history;
        const current = history[this.props.appstate.viewNo];
        const status = "Card " + current.cardCount + " of " + current.maxCards;
        let fwdButton;
        let bckButton;
        if (current.cardCount >= current.maxCards) {
            fwdButton = <CompleteWorkout />;
            bckButton = <PrevCardButton onClick={this.props.handleBackClick} />;
        } else if (current.cardCount === 0) {
            fwdButton = <NewCardButton onClick={this.props.handleForwardClick} />;
            bckButton = null;
        } else {
            fwdButton = <NewCardButton onClick={this.props.handleForwardClick} />;
            bckButton = <PrevCardButton onClick={this.props.handleBackClick} />;
        ...
        if (current.cardCount !== 0) {
            exercise = <this.props.ListExercise currentCard={current.currentCard} suitCount={current.suitCount} />;
        } else {
            exercise = null;
        }
